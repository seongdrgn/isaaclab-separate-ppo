...
from envs.wrappers.base import MultiAgentEnvWrapper, Wrapper
...
class IsaacLabWrapper(Wrapper):
    ...
    def get_input_sequence(self):
        try:
            input_sequence = self._env.get_input_sequence()
        except AttributeError:  # 'OrderEnforcing' object has no attribute 'get_input_sequence'
            input_sequence = self._unwrapped.get_input_sequence()
        
        return input_sequence

    def get_proprioceptive_actions(self):
        try:
            prop_actions = self._env.get_prop_actions()
        except AttributeError:  # 'OrderEnforcing' object has no attribute 'get_prop_action'
            prop_actions = self._unwrapped.get_prop_actions()

        return prop_actions

    def get_images(self):
        try:
            images = self._env.compute_images()
        except AttributeError:  # 'OrderEnforcing' object has no attribute 'state'
            images = self._unwrapped.compute_images()
    ...
    def state(self) -> torch.Tensor:
        ...
        if state is not None:
            return flatten_tensorized_space(tensorize_space(self.state_space, state))
        return state
